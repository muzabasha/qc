...
export default function ModulesLayout({ children }: { children: React.ReactNode }) {
    ...
    // We should also display an instructor profile link.
    return (
        <div className="flex flex-col md:flex-row h-screen bg-slate-950 text-slate-100 overflow-hidden">
            {/* Sidebar */}
            <aside className={`bg-slate-900 border-r border-slate-800 shrink-0 flex flex-col h-full overflow-y-auto z-30 shadow-2xl transition-all duration-300 ${isCollapsed ? 'w-0 md:w-24 overflow-hidden' : 'fixed md:relative inset-y-0 left-0 w-full sm:w-80 md:w-[400px]'}`}>
            ...
            {/* Main Content Area */}
            <main className="flex-1 h-full overflow-y-auto w-full relative overflow-x-hidden scroll-smooth transition-all duration-300">
                {children}
            </main>
        </div>
    )
